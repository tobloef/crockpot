...
export function checkConstraints(permutation, pools) {
...
export function checkNodeConstraints(node, poolName, permutation, pools) {
    const pool = pools.node[poolName] ?? pools.unknown[poolName];
    if (pool === undefined) {
        return false;
    }
...
export function checkEdgeConstraints(edge, poolName, permutation, pools) {
    const pool = pools.edge[poolName] ?? pools.unknown[poolName];
    if (pool === undefined) {
        return false;
    }
    if (pool.constraints.instance !== undefined) {
    ...
    if (pool.constraints.nodes?.toOrFrom !== undefined) {
        ...
        if (secondNode === undefined) {
            ...
            if ((edge.nodes.from !== firstNode || edge.nodes.to !== secondNode) &&
